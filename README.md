...
完成账号初始化并登录管理台后，按以下顺序配置私有实验输出：

1. 进入 `自建节点`，新增一个节点并保持启用状态。节点类型可选择 `vless_ws`；代理路径应使用小写字母、数字和连字符组成的公开 slug，不要使用 UUID 格式，也不要与 `api`、`login`、`register`、`dashboard`、`manage`、`sub`、`health`、`assets`、`convert` 等系统路径冲突。VLESS UUID 必须使用标准 UUID 格式。
2. 进入 `订阅节点`，配置优选 IP 来源。可以在 `自定义优选源 URL` 中填写 HTTPS URL 并保存，再点击从源合并刷新；也可以在手动输入框中直接填写优选 IP 列表并写入缓存。手动格式支持每行一个 `ip`、`ip:port` 或 `ip:port#别名`，端口为空时默认使用 `443`。优选源 URL 不应包含敏感 query 参数。
3. 确认 `订阅内联` 中的 `在订阅中内联优选节点` 已开启。该开关开启后，订阅输出会基于已启用的自建节点模板和已缓存的优选 IP 生成订阅内联节点；若没有启用的自建节点或没有刷新出优选 IP，订阅中不会生成优选节点。
4. 进入 `我的订阅`，复制订阅地址并导入授权实验客户端。页面同时提供默认订阅地址，以及 `Clash`、`Sing-box` 目标格式地址；如重置订阅链接，旧地址会立即失效，需要重新导入客户端。

以上配置仅用于合法、授权、非公开、可控的实验环境。不要公开分享订阅链接、节点链接、邀请码或任何访问凭据；节点或优选 IP 变更后，让授权实验账号在客户端正常更新订阅。
